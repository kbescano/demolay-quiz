...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`questions_options\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`text\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`questions_options_order_idx\` ON \`questions_options\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`questions_options_parent_id_idx\` ON \`questions_options\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`questions\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`number\` numeric,
  	\`active\` integer DEFAULT true,
  	\`keep_option_order\` integer DEFAULT false,
  	\`answer_source\` text DEFAULT 'manual',
  ...
  await db.run(sql`CREATE TABLE \`players_scores\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`players_scores_order_idx\` ON \`players_scores\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`players_scores_parent_id_idx\` ON \`players_scores\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE UNIQUE INDEX \`players_google_sub_idx\` ON \`players\` (\`google_sub\`);`)
  ...
  await db.run(sql`CREATE TABLE \`attempts\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	...
  	\`percent\` numeric,
  	\`completed_at\` text,
  	\`current_index\` numeric DEFAULT 0,
  ...
  `)
  await db.run(sql`CREATE INDEX \`attempts_player_idx\` ON \`attempts\` (\`player_id\`);`)
  await db.run(sql`CREATE INDEX \`attempts_updated_at_idx\` ON \`attempts\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`attempts_created_at_idx\` ON \`attempts\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text DEFAULT 'Petitioners'' Quiz' NOT NULL,
  	\`subtitle\` text DEFAULT 'Order of DeMolay',
  	\`logo_id\` integer,
  	\`seconds_per_question\` numeric DEFAULT 15 NOT NULL,
  	\`updated_at\` text,
  	\`created_at\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`questions_options\`;`)
  await db.run(sql`DROP TABLE \`questions\`;`)
  await db.run(sql`DROP TABLE \`players_scores\`;`)
  await db.run(sql`DROP TABLE \`players\`;`)
  await db.run(sql`DROP TABLE \`attempts\`;`)
